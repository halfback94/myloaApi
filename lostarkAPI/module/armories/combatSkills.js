...
module.exports = {
    request: async (characterName) => {
        let retData = [];
        const url = LostarkApiUrl.armories + '/' + encodeURIComponent(characterName) + '/combat-skills';
        try {
            retData = JSON.parse(await requestAPI.API(url, "GET", null));
        } catch (error) {
            logger.log('error', `armories/character/${characterName}/combat-skills error : ${error}`);
        }
